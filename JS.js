...
// Filter and Map: Get all product titles of products with a price greater than $50.

let ans=(a.map(x=>{if(x.price>50)return x.title})).filter(m=>m!=undefined);
console.log(ans);

//Sort and Find: Find the first product with a category of "electronics" after sorting products by price.
a.sort((c,d)=>c.price-d.price);
// 
console.log(a);
let product=a.find(x=>x.category=="electronics")
// console.log(product);
// Map and Join: Join all product descriptions into a single string separated by commas.
let con=a.map(x=>x.description).join("      ,   ");
console.log(con);
// Filter and Reduce: Calculate the total price of all products with a category of "women's clothing".
let totalpr=a.filter((x)=>x.category==="women's clothing");
console.log(totalpr)
let su=0;
for(let i of totalpr)su+=i.price;
console.log(su);
let fil=a.filter(x=>x.description.includes("waterproof"));
console.log(fil.filter(x=>x.price>20));
